...
class Datachange {
   ...
   public function new($data, $op, $value)
   {
      $dtini = $data.':00';
      $op = $op;
      $valor = abs($value);
      ...
      $this->dtfim = $adi.'-'.str_pad($mdi, 2, "0", STR_PAD_LEFT).'-'.str_pad($ddi, 2, "0", STR_PAD_LEFT).' '.str_pad($hhi, 2, "0", STR_PAD_LEFT).':'.str_pad($mhi, 2, "0", STR_PAD_LEFT).':'.str_pad($shi, 2, "0", STR_PAD_LEFT);
